...
<div class="room-info">
    Ruangan: <strong>{{ $room->code }} — {{ $room->name }}</strong>
    @if($room->description)
        &nbsp;·&nbsp; {{ $room->description }}
    @endif
</div>
...
<table>
    ...
    <tbody>
        ...
        @php
            $latestMoved = $item->latestMovement->moved_at ?? null;
            $displayDate = ($latestMoved ?? $item->purchase_date)
                ? \Carbon\Carbon::parse($latestMoved ?? $item->purchase_date)->format('d M Y')
                : '—';
        @endphp
        <tr>
            <td>{{ $i + 1 }}</td>
